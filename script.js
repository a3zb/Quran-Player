...
if (downloadSurahBtn) {
    downloadSurahBtn.addEventListener('click', () => {
        ...
        // Visual feedback: Start animating
        downloadSurahBtn.classList.add('downloading');
        const icon = downloadSurahBtn.querySelector('i');
        const originalClass = icon.className;
        icon.className = 'fas fa-spinner fa-spin';

        if ('serviceWorker' in navigator && navigator.serviceWorker.controller) {
            // Request Service Worker to cache the audio and video files
            navigator.serviceWorker.controller.postMessage({
                action: 'download',
                url: currentSong.audioSrc
            });
            // We can also download the video if desired, but audio is primary for offline
            if (currentSong.videoBgSrc) {
                navigator.serviceWorker.controller.postMessage({
                    action: 'download',
                    url: currentSong.videoBgSrc
                });
            }

            // Listen for completion
            const messageHandler = (event) => {
                if (event.data.action === 'download-complete' && event.data.url === currentSong.audioSrc) {
                    downloadSurahBtn.classList.remove('downloading');
                    downloadSurahBtn.classList.add('downloaded');
                    icon.className = 'fas fa-check-circle';
                    setTimeout(() => {
                        icon.className = originalClass;
                        downloadSurahBtn.classList.remove('downloaded');
                    }, 3000);
            ...
            alert('خاصية التحميل غير متاحة في هذا المتصفح حالياً.');
            icon.className = originalClass;
            downloadSurahBtn.classList.remove('downloading');
        }
    });
